...
func cleanNameIndices(ctx context.Context, p *populator, sourceID int) error {
...
func processTaxa(
	...
	for rows.Next() {
		...
		useFlat := false
		if cfg.Populate.WithFlatClassification != nil {
			useFlat = *cfg.Populate.WithFlatClassification
		}
		classification, classificationRanks, classificationIDs := getBreadcrumbs(
			taxonID, hierarchy, flatClsf, useFlat,
		)
...
func insertNameIndices(ctx context.Context, p *populator, records [][]any) error {
