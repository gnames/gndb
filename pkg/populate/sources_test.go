...
func TestParseFilename(t *testing.T) {
	tests := []struct {
		...
		{
			name: "with URL",
			filename: "https://example.com/data/" +
				"0025_mydata_2025-03-20_v1.0.sqlite.zip",
			expectedID:      25,
...
`, parentDir)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.False(t, config.DataSources[0].IsOutlinkReady,
				"IsOutlinkReady should be set to false")
		})
	}
}
...
func TestGenerateExampleConfig(t *testing.T) {
	...
	assert.Greater(t, stat.Size(), int64(100),
		"Config file should have substantial content")

	// Verify file is valid YAML (just check it can be read)
	content, err := os.ReadFile(configPath)
	require.NoError(t, err)
	assert.Contains(t, string(content), "data_sources:",
		"Should contain data_sources section")
	assert.Contains(t, string(content), "# sources.yaml",
...
func TestFilterSources_Ranges(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expectError: false,
			description: "Range 10-20 should match IDs 10, 15, 20 " +
				"(silently skip 11-14, 16-19)",
		},
		{
		...
		},
		{
			...
			expectedIDs: nil,
			expectError: true,
			description: "Range with no matching sources should " +
...
`, parentDir, tt.outlinkIDColumn)
			...
			config, err := populate.LoadSourcesConfig(configPath)
			require.NoError(t, err,
				"Should not return error for invalid outlink format: %s",
				tt.outlinkIDColumn)
			require.NotNil(t, config)

			// Should have warnings
			assert.Greater(t, len(config.Warnings), 0,
				"Should have warnings for invalid format: %s",
				tt.outlinkIDColumn)
			if len(config.Warnings) > 0 {
				assert.Contains(t, config.Warnings[0].Message, tt.expectedWarning)
			}
			...
			// Outlink should be disabled
			assert.False(t, config.DataSources[0].IsOutlinkReady,
				"IsOutlinkReady should be disabled")
		})
	}
}
